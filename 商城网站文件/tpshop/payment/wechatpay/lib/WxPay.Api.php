...
class WxPayApi
{
	/**
	 * 
	 * 统一下单，WxPayUnifiedOrder中out_trade_no(订单号)、body(商品描述)、total_fee(标价金额)、trade_type(交易类型)必填
	 * appid(公众账号ID)、mchid(商户号)、spbill_create_ip(终端IP)、nonce_str(随机字符串)不需要填入
	 ...
	 * 
	 * 转换短链接
	 * 该接口主要用于扫码原生支付模式一中的二维码链接转成短链接(weixin://wechatpay/s/XXXXXX)，
	 * 减小二维码数据量，提升扫描速度和精确度。
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	 * @param WxPayConfigInterface $config  配置对象
	...
	{		
		...
		curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,TRUE);// 检查服务器SSL证书中是否存在一个公用名
		curl_setopt($ch,CURLOPT_SSL_VERIFYHOST,2);//严格校验
		curl_setopt($ch,CURLOPT_USERAGENT, $ua); 
		//设置header
		curl_setopt($ch, CURLOPT_HEADER, FALSE);//启用时会将头文件的信息作为数据流输出。false不启动
		//要求结果为字符串且输出到屏幕上
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $sslCertPath);//一个包含PEM格式证书的文件名。
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $sslKeyPath);//包含SSL私钥的文件名。
		}
		//post提交方式
